...
function setCookie(res: Response, name: string, value: string, maxAgeMs: number, path = '/') {
  res.cookie(name, value, {
    httpOnly: true,
    secure: PROD,              // Secure only in production
    sameSite: PROD ? 'none' : 'lax',  // none in production for cross-site, lax locally
    path,
    maxAge: maxAgeMs,
  });
}
...
// Returns current user based on access cookie
...
// Refreshes cookies using refresh token cookie
